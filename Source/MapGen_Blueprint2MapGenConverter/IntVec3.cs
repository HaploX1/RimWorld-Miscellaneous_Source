...
namespace Blueprint2MapGenConverter
{
    public struct IntVec3 : IEquatable<IntVec3>
    {
        ...
        {
            int seed = 0;
            seed = HashCombineInt(seed, this.x);
            seed = HashCombineInt(seed, this.y);
            return HashCombineInt(seed, this.z);
        }
        public static int HashCombineInt(int seed, int value)
        {
            return (int) unchecked ((long)seed ^ (long)value + (long)((ulong)-1640531527) + (long)((long)seed << 6) + (long)(seed >> 2));
        }
