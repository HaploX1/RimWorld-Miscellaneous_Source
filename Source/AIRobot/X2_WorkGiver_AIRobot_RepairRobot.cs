...
namespace AIRobot
{
    public class X2_WorkGiver_AIRobot_RepairRobot : WorkGiver_Scanner
    {
        ...
        {
            if (!(t is X2_Building_AIRobotRechargeStation))
                return false;
            ...
            if (!t1.isRepairRequestActive)
                return false;

            if (pawn is null || pawn.skills is null || pawn.skills.GetSkill(SkillDefOf.Crafting).Level < AIRobot_Helper.jobRepairRobotSkillMin)
                    return false;
